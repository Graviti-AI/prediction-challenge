...
int PyPredictor::PORTNUM = 10000;


PyPredictor::PyPredictor(MapInfo* map, double time_step, double horizon, std::string conda_env, std::string py_path): Predictor(map,time_step,horizon){
    PORTNUM ++;     //TODO: lock and put it outside

    char command[100];
    sprintf(command, "eval \"$(conda shell.bash hook)\"; conda activate %s; python %s --port %d", conda_env.c_str() , py_path.c_str(), PORTNUM);

    printf("PyPredictor construction\n");

    pid_t pid;
    if ((pid = fork()) == 0){
        printf("Simulator: PID %d : %s\n", getpid(), command);
        system(command);
        exit(0);
    }

    printf("Main: Building sockfd %d ....\n", PORTNUM);
    for (; (con = socket(AF_INET, SOCK_STREAM, 0)) == -1;);
    ...
    printf("Main: Connecting sockfd to server %d ....\n", PORTNUM);
    for (; connect(con, (struct sockaddr*) &server_addr, sizeof(struct sockaddr)) < 0; );

    printf("Main: %d Successfully\n", PORTNUM);
}

PredictTra PyPredictor::update(Vector currentState, std::vector<Agent*> agents){
    printf("PyPredictor update\n");
    printf("PyPredictor update\n");
    printf("PyPredictor update\n");
