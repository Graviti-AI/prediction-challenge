...
from predictors.vector_net import VectorNetWithPredicting
...
class VectornetPredictor(Predictor):
    def __init__(self, logger: logging.Logger):
        ...
        print('dev', self.dev)
        self.model = VectorNetWithPredicting(feature_length=14, timeStampNumber=30, device=self.dev).double()
        self.model.to(self.dev)
        self.model.load_state_dict(torch.load(os.path.join(os.path.dirname(__file__), 'vectornet.pt'), map_location=self.dev))
        self.model.eval()
    ...
    def on_env(self, map_name, my_traj: Trajectory, other_trajs: []):
        ...
        self.map_data = RotateOsm(self.map_data, R).to(self.dev)
        test_data = [car_traj, self.map_data, self.map_interval, iR]
        preds = self.model(test_data[0].double(), test_data[1].double(), test_data[2], state='test')
        preds = Local2Global_whole(iR.to(self.dev), preds)
        #########################################
        self.results = (preds.squeeze()).cpu().detach().numpy().copy()
