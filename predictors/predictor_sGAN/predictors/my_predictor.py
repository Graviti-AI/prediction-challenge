...
class SganPredictor(Predictor):
    ...
    def __init__(self, logger: logging.Logger):
        ...
        self._logger = logger
        self.model = get_generator(torch.load(os.path.join(os.path.dirname(__file__), 'sgan.pt'), map_location=self.dev))
    ...
    def on_env(self, map_name, my_traj: Trajectory, other_trajs: []):
        ...
        seq_traj = np.expand_dims(np.transpose(his), axis=0)

        rel_seq_traj = np.zeros(seq_traj.shape)
        rel_seq_traj[:, :, 1:] = seq_traj[:, :, 1:] - seq_traj[:, :, :-1]

        seq_traj = torch.from_numpy(seq_traj).type(torch.float).permute(2, 0, 1).to(self.dev)
        rel_seq_traj = torch.from_numpy(rel_seq_traj).type(torch.float).permute(2, 0, 1).to(self.dev)

        seq_start_end = [[0, 1]]
        seq_start_end = torch.LongTensor(seq_start_end).to(self.dev)

        #####################
        pred_traj_fake_rel = self.model(
            seq_traj, rel_seq_traj, seq_start_end
        )
        pred_traj_fake = relative_to_abs(
            pred_traj_fake_rel, seq_traj[-1]
        )

        pred_abs = pred_traj_fake.squeeze().detach().cpu().numpy()

        assert pred_abs.shape == (30, 2)
        ####################

        self.results = pred_abs.copy()
...
class Encoder(nn.Module):
    ...
    def init_hidden(self, batch):
        return (
            torch.zeros(self.num_layers, batch, self.h_dim).to(self.dev),
            torch.zeros(self.num_layers, batch, self.h_dim).to(self.dev)
        )
...
class TrajectoryGenerator(nn.Module):
    ...
    def add_noise(self, _input, seq_start_end, user_noise=None):
        ...
        if user_noise is not None:
            ...
            z_decoder = get_noise(noise_shape, self.noise_type).to(self.dev)
