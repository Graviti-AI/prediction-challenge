    ...
    for key, value in track_dictionary.items():
        ...
        for timestamp in value.motion_states.keys():
            ...
            if (ms.x - config.EgoEndPositionX)**2+(ms.y - config.EgoEndPositionY)**2 <= 1 and not efficiency_flag:
                metrics['efficiency'] =  (config.EndframeTimestamp-config.StartframeTimestamp) - (timestamp - value.time_stamp_ms_first)
...
def score_of_metrics(metrics):
    return -metrics['jerk']-metrics['velo']-(1-metrics['courtesy'])*100000000+metrics['efficiency']+metrics['courtesy']
