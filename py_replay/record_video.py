...
def update_plot(timestamp, fig, title_text, patches_dict, text_dict, axes):
...
def start_playback(timestamp_min, timestamp_max, c):
    ...
    for timestamp in range(timestamp_min, timestamp_max, step):
        update_plot(timestamp, fig, title_text, patches_dict, text_dict, axes)
    ...
    ims[0].save(os.path.join('visualization', args.l, c + '.gif'), save_all=True, append_images=ims[1:], optimize=False, duration=100, loop=0)
